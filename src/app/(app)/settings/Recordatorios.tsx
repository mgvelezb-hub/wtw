...
import { sincronizarAvisosLocales, useEsNativo } from "@/lib/nativo";
import { AvisosNativos } from "./Nativo";
...
export function RecordatoriosPanel({
  ...
  // Dentro del cascarón, guardar un horario reprograma el dispositivo: el
  // puente vuelve a pedir el plan al servidor y `sello` hace que la lista de
  // pendientes se relea.
  function guardar(next: Recordatorios) {
    ...
    startTransition(async () => {
      ...
      if (nativo) {
        await sincronizarAvisosLocales({ forzar: true }).catch(() => {});
        setSello((s) => s + 1);
      }
    });
  }
  ...
  async function activarEnEsteDispositivo() {
    ...
    if (!("serviceWorker" in navigator) || !("PushManager" in window)) {
      setEstado(
        "Este navegador no soporta notificaciones push. En iPhone y iPad hay que agregar la app a la pantalla de inicio primero.",
      );
      return;
    }
    if (!clavePublica) {
    ...
    try {
      ...
      const res = await fetch("/api/v1/push", {
        ...
        body: JSON.stringify({
          ...sub.toJSON(),
          etiqueta: navigator.userAgent.slice(0, 80),
        }),
      });
    ...
    <section className="space-y-4 rounded-lg border border-edge bg-surface p-6">
      <div>
        ...
        <p className="mt-1 text-xs leading-relaxed text-muted">
          Dos avisos, y ninguno llega si la cosa ya está hecha: el del ritual
          solo si la semana que entra sigue sin plan, el del cierre solo si
          quedan minutos del día sin explicar.
        </p>
      </div>

      {!nativo && tickUnico && (
        <p className="text-xs leading-relaxed text-muted">
          El plan de Vercel permite un solo disparo al día, así que hoy los
          avisos salen a las <span className="num">17:30</span> y la hora que
          elijas abajo queda guardada pero no se usa. El día sí se respeta.
        </p>
      )}

      {!nativo && !configurado && (
        <p className="rounded-md bg-warn-soft px-3 py-2 text-xs text-warn">
          Push sin configurar en este despliegue: faltan{" "}
          <span className="num">VAPID_PUBLIC_KEY</span> y{" "}
          <span className="num">VAPID_PRIVATE_KEY</span>. Los horarios se pueden
          guardar, pero no llegará nada.
        </p>
      )}
      ...
      <label className="flex flex-wrap items-center gap-2 text-sm">
        <input
          ...
          onChange={(e) =>
            guardar({
              ...r,
              ritual: e.target.checked ? { dia: 0, hora: "18:00" } : null,
            })
          }
        />
          ...
          <>
            <select
              ...
              onChange={(e) =>
                guardar({
                  ...r,
                  ritual: { ...r.ritual!, dia: Number(e.target.value) },
                })
              }
            ...
            <input
              ...
              onChange={(e) =>
                guardar({
                  ...r,
                  ritual: { ...r.ritual!, hora: e.target.value },
                })
              }
      ...
      <label className="flex flex-wrap items-center gap-2 text-sm">
        <input
          ...
          onChange={(e) =>
            guardar({
              ...r,
              cierre: e.target.checked ? { hora: "17:30" } : null,
            })
          }
        />
        ...
        {r.cierre && (
          <input
            ...
            onChange={(e) =>
              guardar({ ...r, cierre: { hora: e.target.value } })
            }
            className="num rounded border border-hair px-1.5 py-1 text-xs text-ink"
          />
        )}
